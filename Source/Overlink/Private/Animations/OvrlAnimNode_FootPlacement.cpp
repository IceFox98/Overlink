...
#include "Animations/OvrlAnimNode_FootPlacement.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OvrlAnimNode_FootPlacement)

DECLARE_CYCLE_STAT(TEXT("Foot Placement Eval"), STAT_OvrlFootPlacement_Eval, STATGROUP_Anim);

#if ENABLE_FOOTPLACEMENT_DEBUG
static TAutoConsoleVariable<bool> CVarOvrlAnimNodeFootPlacementEnable(TEXT("Ovrl.FootPlacement.Enable"), true, TEXT("Enable/Disable Foot Placement"));
static TAutoConsoleVariable<bool> CVarOvrlAnimNodeFootPlacementEnableLock(TEXT("Ovrl.FootPlacement.Enable.Lock"), true, TEXT("Enable/Disable Foot Locking"));
static TAutoConsoleVariable<bool> CVarOvrlAnimNodeFootPlacementDebug(TEXT("Ovrl.FootPlacement.Debug"), false, TEXT("Turn on visualization debugging for Foot Placement"));
static TAutoConsoleVariable<bool> CVarOvrlAnimNodeFootPlacementDebugTraces(TEXT("Ovrl.FootPlacement.Debug.Traces"), false, TEXT("Turn on visualization debugging for foot ground traces"));
static TAutoConsoleVariable<int> CVarOvrlAnimNodeFootPlacementDebugDrawHistory(TEXT("Ovrl.FootPlacement.Debug.DrawHistory"), 0,
...
namespace UE::Anim::FootPlacement
{
	...
	static FVector PointDirectionPlaneIntersection(const FVector Point, const FVector Direction, const FPlane Plane)
	{
		...
		const float DotProduct = Direction | Plane;
    
		// If the direction is parallel to the plane, they never intersect. 
		// Fallback to the point to prevent a NaN division.
		if (FMath::IsNearlyZero(DotProduct, KINDA_SMALL_NUMBER))
		{
			return Point; 
		}
...
#if ENABLE_FOOTPLACEMENT_DEBUG
		if (CVarOvrlAnimNodeFootPlacementDebugTraces.GetValueOnAnyThread())
		{
...
void FOvrlAnimNode_FootPlacement::FindPelvisOffsetRangeForLimb(
...
float FOvrlAnimNode_FootPlacement::CalcTargetPlantPlaneDistance(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
...
void FOvrlAnimNode_FootPlacement::AlignPlantToGround(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
...
FTransform FOvrlAnimNode_FootPlacement::UpdatePlantOffsetInterpolation(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
...
void FOvrlAnimNode_FootPlacement::UpdatePlantingPlaneInterpolation(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	...
	UE::Anim::FootPlacement::FLegRuntimeData::FInterpolationData& InOutInterpData) const
{
	...
	if (InterpolationSettings.bEnableFloorInterpolation && !bIsFirstUpdate)
	{
		...
		*/
		...
		// Calculate the distance along the custom gravity normal (-ApproachDirWS is "Up")
		const FVector UpDirectionWS = -Context.ApproachDirWS;

		const float LastPlaneDeltaVertical = (LastPlaneIntersection - CurrPlaneIntersection) | UpDirectionWS;
		const float PrevPlaneDeltaVertical = (PrevPlaneIntersection - CurrPlaneIntersection) | UpDirectionWS;

		float AdjustedPrevVertical = FMath::Abs(LastPlaneDeltaVertical) < FMath::Abs(PrevPlaneDeltaVertical) ?
			(LastPlaneIntersection | UpDirectionWS) : (PrevPlaneIntersection | UpDirectionWS);

		if (CharacterData.bIsOnGround)
		{
			const float GroundPlaneDelta = (CurrPlaneIntersection | UpDirectionWS) - AdjustedPrevVertical;
			const float VerticalMoveDelta = CharacterData.ComponentMoveDeltaWS | UpDirectionWS;
    
			if ((GroundPlaneDelta) > 0.0f)
			{
				AdjustedPrevVertical += FMath::Min(FMath::Abs(-VerticalMoveDelta), GroundPlaneDelta);
			}
			else
			{
				AdjustedPrevVertical += FMath::Max(-FMath::Abs(VerticalMoveDelta), GroundPlaneDelta);
			}
		}

		float PlantPlaneSpringHeight = UKismetMathLibrary::FloatSpringInterp(
			AdjustedPrevVertical, (CurrPlaneIntersection | UpDirectionWS), InOutInterpData.GroundHeightSpringState,
			InterpolationSettings.FloorLinearStiffness,
			InterpolationSettings.FloorLinearDamping,
			Context.UpdateDeltaTime, 1.0f, 0.0f);

		// Reconstruct the current plane intersection point along the new vertical height
		CurrPlaneIntersection = CurrPlaneIntersection - (UpDirectionWS * ((CurrPlaneIntersection | UpDirectionWS) - PlantPlaneSpringHeight));
		...
		if ((TraceSettings.MaxGroundPenetration >= 0.0f) && bFoundGround)
		{
			...
			const TOptional<float> DistanceToGroundPlane =
				UE::Anim::FootPlacement::GetDistanceToPlaneAlongDirection(CurrPlaneIntersection, GroundPlane, Context.ApproachDirCS);
			const float PenetrationAmount = -DistanceToGroundPlane.Get(0.0f) - TraceSettings.MaxGroundPenetration;
...
void FOvrlAnimNode_FootPlacement::DeterminePlantType(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
...
float FOvrlAnimNode_FootPlacement::GetMaxLimbExtension(const float DesiredExtension, const float LimbLength) const
{
...
float FOvrlAnimNode_FootPlacement::GetMinLimbExtension(const float DesiredExtension, const float LimbLength) const
{
	return FMath::Min(DesiredExtension, LimbLength * PlantSettings.MinExtensionRatio);
}

void FOvrlAnimNode_FootPlacement::ResetRuntimeData() 
{
...
bool FOvrlAnimNode_FootPlacement::WantsToPlant(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	const UE::Anim::FootPlacement::FLegRuntimeData::FInputPoseData& LegInputPose) const
{
#if ENABLE_FOOTPLACEMENT_DEBUG
	if (!CVarOvrlAnimNodeFootPlacementEnableLock.GetValueOnAnyThread())
	{
		return false;
	}
#endif
...
float FOvrlAnimNode_FootPlacement::GetAlignmentAlpha(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	const UE::Anim::FootPlacement::FLegRuntimeData::FInputPoseData& LegInputPose) const
{
	return FMath::Clamp(
...
FTransform FOvrlAnimNode_FootPlacement::GetFootPivotAroundBallWS(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	const UE::Anim::FootPlacement::FLegRuntimeData::FInputPoseData& LegInputPose,
...
UE::Anim::FootPlacement::FPlantResult FOvrlAnimNode_FootPlacement::FinalizeFootAlignment(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	...
	const FTransform& PelvisTransformCS)
{
	...
	const TOptional<float> FootDistance = UE::Anim::FootPlacement::GetDistanceToPlaneAlongDirection(
		CorrectedBallLocationCS,
		PlantPlaneCS,
		Context.ApproachDirCS);
	const TOptional<float> BallDistance = UE::Anim::FootPlacement::GetDistanceToPlaneAlongDirection(
...
#if ENABLE_FOOTPLACEMENT_DEBUG
void FOvrlAnimNode_FootPlacement::DrawVLog(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	const UE::Anim::FootPlacement::FLegRuntimeData& LegData,
...
#if ENABLE_FOOTPLACEMENT_DEBUG
void FOvrlAnimNode_FootPlacement::DrawDebug(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	const UE::Anim::FootPlacement::FLegRuntimeData& LegData,
	const UE::Anim::FootPlacement::FPlantResult& PlantResult) const
{
...
FOvrlAnimNode_FootPlacement::FOvrlAnimNode_FootPlacement()
{
}

// TODO: implement 
void FOvrlAnimNode_FootPlacement::GatherDebugData(FNodeDebugData& NodeDebugData)
{
	ComponentPose.GatherDebugData(NodeDebugData);
}

void FOvrlAnimNode_FootPlacement::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	FAnimNode_SkeletalControlBase::Initialize_AnyThread(Context);
	ResetRuntimeData();
}

void FOvrlAnimNode_FootPlacement::UpdateInternal(const FAnimationUpdateContext& Context)
{
...
void FOvrlAnimNode_FootPlacement::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output,
	TArray<FBoneTransform>& OutBoneTransforms)
{
	SCOPE_CYCLE_COUNTER(STAT_OvrlFootPlacement_Eval);
	...
	FVector GravityDirWS = -FVector::UpVector; // Default fallback

	if (!CustomGravityDirection.IsZero())
	{
		GravityDirWS = CustomGravityDirection;
	}

	// Convert the World Space gravity direction to Component Space
	const FVector ApproachDirCS = Output.AnimInstanceProxy->GetComponentTransform().InverseTransformVectorNoScale(GravityDirWS).GetSafeNormal();
	UE::Anim::FootPlacement::FEvaluationContext FootPlacementContext(Output, ApproachDirCS, CachedDeltaTime);
	...
	if (CVarOvrlAnimNodeFootPlacementDebug.GetValueOnAnyThread())
	{
		// Debug text header
...
#if ENABLE_FOOTPLACEMENT_DEBUG
	{
		...
		if (CVarOvrlAnimNodeFootPlacementDebug.GetValueOnAnyThread())
		{
			const int32 DrawIndex = CVarOvrlAnimNodeFootPlacementDebugDrawHistory.GetValueOnAnyThread();
...
bool FOvrlAnimNode_FootPlacement::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
#if ENABLE_FOOTPLACEMENT_DEBUG
	if (!CVarOvrlAnimNodeFootPlacementEnable.GetValueOnAnyThread())
	{
		return false;
	}
#endif
...
void FOvrlAnimNode_FootPlacement::InitializeBoneReferences(const FBoneContainer& RequiredBones)
{
	for (int32 FootIndex = 0; FootIndex < LegsData.Num(); ++FootIndex)
	{
...
void FOvrlAnimNode_FootPlacement::GatherPelvisDataFromInputs(const UE::Anim::FootPlacement::FEvaluationContext& Context)
{
...
void FOvrlAnimNode_FootPlacement::GatherLegDataFromInputs(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	UE::Anim::FootPlacement::FLegRuntimeData& LegData,
	const FFootPlacemenLegDefinition& LegDef)
{
...
void FOvrlAnimNode_FootPlacement::CalculateFootMidpoint(const UE::Anim::FootPlacement::FEvaluationContext& Context, TConstArrayView<UE::Anim::FootPlacement::FLegRuntimeData> InLegsData, FVector& OutMidpoint) const
{
...
void FOvrlAnimNode_FootPlacement::ProcessCharacterState(const UE::Anim::FootPlacement::FEvaluationContext& Context)
{
...
void FOvrlAnimNode_FootPlacement::ProcessFootAlignment(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
	UE::Anim::FootPlacement::FLegRuntimeData& LegData)
{
	...
	if (PlantSettings.SeparatingDistance > 0.0f)
	{
		...
		if (CVarOvrlAnimNodeFootPlacementDebug.GetValueOnAnyThread())
		{
			...
			DebugData.LegsInfo[LegData.Idx].DistanceToSeparatingPlane = DistanceToSeparatingPlane.Get(0.0f);
		}
#endif
...
FVector FOvrlAnimNode_FootPlacement::GetApproachDirWS(const FAnimationBaseContext& Context) const
{
...
const FTransform& FOvrlAnimNode_FootPlacement::GetRootToComponent() const
{
	return PelvisData.InputPose.RootTransformCS;
}

FTransform FOvrlAnimNode_FootPlacement::SolvePelvis(const UE::Anim::FootPlacement::FEvaluationContext& Context)
{
	using namespace UE::Anim::FootPlacement;
...
FTransform FOvrlAnimNode_FootPlacement::UpdatePelvisInterpolationRootSpace(
	const UE::Anim::FootPlacement::FEvaluationContext& Context,
