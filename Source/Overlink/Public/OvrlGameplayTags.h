// Copyright Epic Games, Inc. All Rights Reserved.

#pragma once

#include "NativeGameplayTags.h"

namespace OvrlGameplayMessageTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(EnemyKilled);
};

namespace OvrlDataTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Damage);
}

namespace OvrlAbilityTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ActivateFail_IsDead);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ActivateFail_Cooldown);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ActivateFail_Cost);
	
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(PlayerRun);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(PlayerJump);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WeaponFire);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WeaponReload);
};

namespace OvrlLocomotionModeTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Grounded);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(InAir);
};

namespace OvrlGaitTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Idle);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Walking);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Running);
};

namespace OvrlLocomotionActionTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Mantling); // Grab a high edge and pull up
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Vaulting); // Jump either over or on an obstacle
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Ragdolling);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Sliding);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WallrunningVertical);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WallrunningLeft);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WallrunningRight);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WallClinging); // Wallrun "idle", when character is clinging to a wall after a vertical wallrun
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(GettingUp);
};

namespace OvrlViewModeTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ADS);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Relaxed);
}

namespace OvrlInputTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Move);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Run);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Look_Mouse);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Crouch);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Jump);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Slide);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Interact);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Fire);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Reload);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ADS);
};

namespace OvrlStanceTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Standing);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Crouching);
};

namespace OvrlOverlayModeTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Default);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Pistol);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Rifle);
};

namespace OvrlWeaponTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Reloading);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ReloadCompleted);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ReloadFailed);

	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(MagazineAmmo);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(MagazineSize);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(SpareAmmo);
}

namespace OvrlCheatTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(GodMode);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(UnlimitedHealth);
};

namespace OvrlUILayerTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(LayerGame);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(LayerMenu);
};

namespace OvrlGameplayCueTags
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WeaponFire);
};

namespace OvrlFoleyEvents
{
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Walk);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WalkBackwards);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(WalkStrafe);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Run);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(RunBackwards);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(RunStrafe);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Jump);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Slide);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Scuff);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ScuffWall);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Handplant);
	OVERLINK_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Land);
};